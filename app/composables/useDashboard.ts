...
export const useDashboard = () => {
  ...
  const loadReal = async () => {
    const [data, top] = await Promise.all([
      ...
      $api<TopProductsResponse>("/dashboard/top-products?limit=20").catch(
        () => ({ items: [] as DashboardTopProduct[] }),
      ),
    ]);

    // Map recent transactions into the shape index.vue expects
    recentTransactions.value = (data.recentTransactions ?? []).map((tx) => ({
      id: tx.id, // ← remove .slice(0, 8)
    ...
    stats.value = [
      ...
      },
      {
        ...
        trend: {
          value: lowStockCount > 0 ? "action needed" : "all clear",
          up: lowStockCount === 0,
        },
      },
    ];
  ...
  const loadMock = async () => {
    ...
    const totalRevenue = transactions
      .filter(
        (t) => t.payment_status === "paid" || t.payment_status === "overpaid",
      )
      .reduce((sum, t) => sum + t.total_paid, 0);
    const pendingTx = transactions.filter((t) => t.status === "pending").length;
    const unpaidAmount = transactions
      .filter(
        (t) => t.payment_status === "partial" || t.payment_status === "unpaid",
      )
    ...
    stats.value = [
      ...
      },
      {
        ...
        color: "amber",
        trend: {
          value: pendingTx > 0 ? "needs action" : "all clear",
          up: false,
        },
      },
      {
        ...
        icon: "products",
        color: outOfStock > 0 ? "red" : "slate",
        trend: {
          value: outOfStock > 0 ? `${outOfStock} OOS` : "stock OK",
          up: outOfStock === 0,
        },
      },
    ];
